...
function($, config, utils, messageTpl, cards, uuidv1){
	...
	class ApiHandler{
		...
		constructor(ses){
			console.log(config.chatSession);
			//config.chatSession = $("#chatSess").val();
			//alert(config.chatSession);
			this.options = {
				sessionId: config.chatSession,
				lang: "en"
			};
		}
		...
		askBot(userInput, callback){			
			...
			config.chatSession = $("#chatSess").val();
			this.options.sessionId = config.chatSession;
